...
int mainCuda(int argc, char** argv)
{
	...
	int blockDem = 128;
	int gridDem = ceil(static_cast<double>(inputLength) / blockDem);

	wbTime_start(Compute, "Performping CUDA computation");
	//@@ Launch the GPU Kernel here
	//vecAdd <<< gridDem, blockDem >>>(deviceInput1, deviceInput2, deviceOutput, inputLength);
